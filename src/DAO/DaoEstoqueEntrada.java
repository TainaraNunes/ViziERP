...
public class DaoEstoqueEntrada {

    ConectaBD conecta = new ConectaBD();
    BeansEntradaEstoque lanca = new BeansEntradaEstoque();

    public int gerarPlanilhaEntrada() {
        int novaPlanilha = 0;

        conecta.conexao();
        conecta.executaSql("SELECT PLANILHA, OPERACAO FROM ESTOQUE WHERE OPERACAO = '0001' ORDER BY PLANILHA DESC LIMIT 1");
        try {
            if (conecta.rs.next()) {
    ...
    public void incluirProdutoTabela(EstoqueEntrada estoque) {
    ...
    public void alterarProdutoTabela(EstoqueEntrada estoque) {
    ...
    public void excluirProdutoTabela(EstoqueEntrada entrada){
        conecta.conexao();
        try {
            PreparedStatement pst = conecta.con.prepareStatement("DELETE FROM ESTOQUE WHERE PLANILHA = ? AND PRODUTOCODIGO = ?");
            pst.setInt(1, Integer.parseInt(entrada.txtPlanilha.getText()));
            pst.setInt(2, Integer.parseInt(entrada.txtCodigoProduto.getText()));
            pst.execute();
            saldoEstoque(Integer.parseInt(entrada.txtCodigoProduto.getText()));
        } catch (SQLException ex) {
    ...
    public void gravarEntradaEstoque(BeansEntradaEstoque lanca, EstoqueEntrada prodTabela) {
        conecta.conexao();
        try {
            ...
            PreparedStatement pst = conecta.con.prepareStatement("INSERT INTO ESTOQUE (PLANILHA, OPERACAO, DATALANCAMENTO, PESSOACODIGO, PESSOADESCRICAO, NOTAFISCAL, PRODUTOCODIGO, PRODUTODESCRICAO, QUANTIDADE, VALORUNITARIO, VALORTOTAL, USUARIO) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            PreparedStatement pst3 = conecta.con.prepareStatement("UPDATE PRODUTOS SET DATAULTIMACOMPRA = ?, CUSTOULTIMACOMPRA = ? WHERE CODIGO = ?");

            int imprimirPlanilha = 0;
            if (prodTabela.incluindo == 1) {
                pst.setInt(1, gerarPlanilhaEntrada());
                imprimirPlanilha = gerarPlanilhaEntrada();
            } else {
            ...
            for (int x = 0; x < prodTabela.tblProdutosEstoque.getRowCount(); x++) {
              ...
              //Atualiza a data e o custo da última compra no cadastro do produto.
                pst3.setDate(1, java.sql.Date.valueOf(lanca.getDataLancamento()));
                pst3.setBigDecimal(2, valorUnitario);
                pst3.setInt(3, Integer.parseInt(String.valueOf(prodTabela.tblProdutosEstoque.getValueAt(x, 0))));
                pst3.execute();
    ...
    public void excluirLancamentoEntradaEstoque(BeansEntradaEstoque lanca) {
        conecta.conexao();
        try {
